...
static int vmwgfx_init(struct driver *drv)
{
  ...
  drv_modify_combination(drv, DRM_FORMAT_NV12, &LINEAR_METADATA, BO_USE_SCANOUT | BO_USE_SW_MASK |  BO_USE_LINEAR |
                                 BO_USE_CAMERA_READ | BO_USE_CAMERA_WRITE | BO_USE_HW_VIDEO_DECODER |
                                 BO_USE_HW_VIDEO_ENCODER);
  drv_modify_combination(drv, DRM_FORMAT_YUYV, &LINEAR_METADATA, BO_USE_HW_VIDEO_DECODER | BO_USE_SCANOUT | BO_USE_LINEAR |
          ...
          uint32_t format, const uint64_t *modifiers, uint32_t count)
{
  ...
  if (ret) {
	  drv_loge("DRM_VMW_GB_SURFACE_CREATE failed (%d, %d), req format: 0x%x, drm format: 0x%x\n", bo->drv->fd, ret, req->format, format);
	  return ret;
  }
			    ...
			    uint64_t use_flags)
{
  ...
  return vmwgfx_bo_create_with_modifiers(bo, width, height, format, modifiers, ARRAY_SIZE(modifiers));
}
